...
- **Stop the turn.** Throw `TurnRefused` with a message: nothing is sent to the provider, nothing is stored, and the turn ends `refused` with the message for the person to read.
